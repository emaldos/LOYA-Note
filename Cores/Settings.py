...
class TargetValues:
    ...
    def export_json(self,out_path,data):_write_json(out_path,{k:{"priority":int((v or {}).get("priority",(v or {}).get("value",0)) or 0)} for k,v in (data or {}).items()})
    ...
    def apply_plan(self,base,plan,decisions):
        ...
        for k,v in (plan.get("new") or {}).items():
            base[_norm(k)]={"priority":int((v or {}).get("priority",(v or {}).get("value",0)) or 0)};added+=1
        for idx,d in enumerate(plan.get("dups") or []):
            act=decisions.get(idx,"Skip")
            if act=="Skip":skipped+=1;continue
            exk=d.get("existing_key","")
            if act=="Replace":
                base[exk]={"priority":int((d.get("incoming") or {}).get("priority",(d.get("incoming") or {}).get("value",0)) or 0)};replaced+=1
            else:
                del base[exk]
                base[_norm(d.get("key",""))]={"priority":int((d.get("incoming") or {}).get("priority",(d.get("incoming") or {}).get("value",0)) or 0)};overwritten+=1
