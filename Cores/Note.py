...
from PyQt6.QtGui import QIcon,QKeySequence,QTextCharFormat,QTextListFormat,QTextTableFormat,QTextCursor,QShortcut,QAction,QColor,QTextBlockFormat,QImage,QTextImageFormat,QTextFormat,QTextLength,QTextDocumentFragment,QSyntaxHighlighter
from PyQt6.QtWidgets import QWidget,QVBoxLayout,QHBoxLayout,QFrame,QLabel,QLineEdit,QToolButton,QTextEdit,QMessageBox,QDialog,QGridLayout,QSpinBox,QTabWidget,QTableWidget,QTableWidgetItem,QHeaderView,QAbstractItemView,QMenu,QComboBox,QFileDialog,QInputDialog,QSplitter,QCompleter,QListWidget,QApplication
...
class Widget(QWidget):
    ...
    def _build_create(self):
        ...
        for b in (self.btn_add,self.btn_pick,self.btn_clear,self.btn_save):
        ...
        top.addWidget(self.in_name,1);top.addWidget(self.btn_add,0);top.addWidget(self.btn_pick,0);top.addWidget(self.btn_clear,0);top.addWidget(self.btn_save,0)
    ...
    def _on_list_cell(self,row,col):
        ...
        if col in (1,2,3):return self._load_into_editor(n)
        if col==4:
            w=self.window() if self.window() else self
            name=(n.get("note_name","") or "").strip()
